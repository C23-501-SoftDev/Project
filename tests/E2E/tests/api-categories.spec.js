...
test.describe("USERS API", () => {
  test("USERS-API-01: create, soft-delete, restore user lifecycle", async ({
    ...
    const withDeleted = await adminApi.get(
      `${baseURL}/api/admin/users?page=0&size=200&includeDeleted=true`
    );
...
test.describe("SPACES API", () => {
  ...
  test("SPACES-API-05: full permission matrix for guest/reader/editor", async ({
    ...
    // GUEST + WRITE
    // NOTE: current SecurityConfig blocks GUEST for POST/PUT /api/documents
    // at filter-chain level, even with explicit WRITE permission on space.
    ...
    expect(guestCreateDoc.status()).toBe(403);

    const guestUpdateDoc = await guestWriteApi.put(`${baseURL}/api/documents/${seedDoc.id}`, {
      data: { title: `Guest write doc ${suffix} updated`, content: "updated" },
    });
    expect(guestUpdateDoc.status()).toBe(403);
