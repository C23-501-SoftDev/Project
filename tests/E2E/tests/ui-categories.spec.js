...
test.describe("SPACES UI", () => {
  ...
  test("SPACES-UI-04: admin panel creates users/space then access is validated", async ({
    browser,
    request,
    baseURL,
  }) => {
    const suffix = uniqueSuffix();
    const guestLogin = `ui_guest_${suffix}`;
    const readerLogin = `ui_reader_${suffix}`;
    const editorLogin = `ui_editor_${suffix}`;
    const guestPass = "GuestPass123!";
    const readerPass = "ReaderPass123!";
    const editorPass = "EditorPass123!";
    const spaceName = `UI Matrix Space ${suffix}`;
    ...
    await expect(userRow(page, guestLogin)).toHaveCount(1);
    await expect(userRow(page, readerLogin)).toHaveCount(1);
    await expect(userRow(page, editorLogin)).toHaveCount(1);

    // 2) Create space from admin panel
    await page.goto("/admin/spaces");
    await page.getByRole("button", { name: /создать пространство/i }).click();
    await page.locator("#spaceName").fill(spaceName);
    await page.locator("#spaceDesc").fill("Space from UI category scenario");
    await selectFirstSpaceOwner(page);
    const createSpaceRes = page.waitForResponse(
      (r) => r.url().includes("/api/admin/spaces") && r.request().method() === "POST"
    );
    await page.locator("#saveSpaceBtn").click();
    const createdSpaceResponse = await createSpaceRes;
    expect(createdSpaceResponse.status()).toBe(201);
    const createdSpace = await createdSpaceResponse.json();
    await context.close();

    // 3) Grant permissions via API and validate by user sessions
    const adminApi = await createAdminApi(baseURL);
    const usersPage = await adminApi.get(
      `${baseURL}/api/admin/users?page=0&size=200&includeDeleted=false`
    );
    const usersPayload = await usersPage.json();
    const guest = usersPayload.content.find((u) => u.login === guestLogin);
    const reader = usersPayload.content.find((u) => u.login === readerLogin);
    expect(guest).toBeTruthy();
    expect(reader).toBeTruthy();

    const grantGuestWrite = await adminApi.post(
      `${baseURL}/api/admin/spaces/${createdSpace.id}/permissions`,
      { data: { userId: guest.id, permissionType: "WRITE" } }
    );
    expect(grantGuestWrite.status()).toBe(201);

    const grantReaderRead = await adminApi.post(
      `${baseURL}/api/admin/spaces/${createdSpace.id}/permissions`,
      { data: { userId: reader.id, permissionType: "READ" } }
    );
    expect(grantReaderRead.status()).toBe(409);

    const seedDocRes = await adminApi.post(`${baseURL}/api/documents`, {
      data: {
        title: `UI matrix seed ${suffix}`,
    ...
    const guestApiDirect = await playwrightRequest.newContext({ baseURL });
    const guestLoginRes = await guestApiDirect.post(`${baseURL}/api/auth/login`, {
      data: { login: guestLogin, password: guestPass },
    });
    expect(guestLoginRes.ok()).toBeTruthy();
    const guestCreate = await guestApiDirect.post(`${baseURL}/api/documents`, {
      data: {
        title: `UI guest write ${suffix}`,
        spaceId: createdSpace.id,
        content: "created by guest write",
      },
    });
    // Current backend security allows POST /api/documents only for ADMIN/EDITOR/READER.
    // GUEST is denied at security layer even with explicit WRITE permission.
    expect(guestCreate.status()).toBe(403);
    await guestApiDirect.dispose();

    const readerApiDirect = await playwrightRequest.newContext({ baseURL });
    const readerLoginRes = await readerApiDirect.post(`${baseURL}/api/auth/login`, {
      data: { login: readerLogin, password: readerPass },
    });
    expect(readerLoginRes.ok()).toBeTruthy();
    const readerRead = await readerApiDirect.get(`${baseURL}/api/documents/${seedDoc.id}`);
    expect(readerRead.ok()).toBeTruthy();
    const readerCreate = await readerApiDirect.post(`${baseURL}/api/documents`, {
      data: { title: `UI reader create ${suffix}`, spaceId: createdSpace.id, content: "x" },
    });
    expect([401, 403]).toContain(readerCreate.status());
    await readerApiDirect.dispose();
  });
});
